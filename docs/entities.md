...
| `switch.a32_pro_coolant_blower_mode_auto_manual` | Blower Auto (PID) / Manual; Manual holds while the thermostat heats, until it is next switched on |
...
| `number.a32_pro_hydronic_blower_start_temp` | Coolant temp the PID blower waits for (default 60 °C; stops 10 °C below) |
...
| `sensor.a32_pro_coolant_blower_heating_pid_climate_result` | PID output in % — raw demand, often far above 100 while the cabin is well below target; the blower clamps at 100 % |
...
| `sensor.propane_tank_percentage` | Template: calculated % — unavailable while the Mopeka sensor is (dead coin cell 2026-07-23) |
